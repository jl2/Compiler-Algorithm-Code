...
dfa_move st ch fa = lookupDefault (lookupDefault (d_transitions fa) Map.empty st) 0 ch
...
visit re pt =
    ...
    in
      -- Construct the nfa
      Nfa (n_transitions nf) 0 (Set.insert lastState (n_accepting nf)) (n_rgx nf)

-- Return the map of NfaChar -> StateSet for state st of Nfa fa
getCharMap :: State -> Nfa -> (Map.Map NfaChar (StateSet))
getCharMap st fa = lookupDefault (n_transitions fa) Map.empty st
...
getTrans st ch fa =
    let oldTrans = (n_transitions fa)
        innerMap = lookupDefault oldTrans Map.empty st
        goesTo = lookupDefault innerMap Set.empty ch
    in
      goesTo
...
removeState st fa =
    Nfa newTrans  (n_start fa) (n_accepting fa) (n_rgx fa)
    where
...
lookupDefault :: (Ord a) => (Map.Map a b) -> b -> a -> b
lookupDefault theMap def key =
...
toDfa nfa =
    let
        ...
        partial = Map.mapKeys (\x -> lookupDefault stateNames 0 x) dfa
        dfaTrans = Map.map (\x -> Map.map (\y -> (lookupDefault stateNames 0 y)) x) partial
